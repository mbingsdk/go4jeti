...
func ConnectPoll() *core.TalkServiceClient {
	...
	connect.SetHeader("User-Agent", "LLA/2.16.0 SMJ500G 5.1.1")
	connect.SetHeader("X-Line-Application", "ANDROIDLITE\t2.15.0\tAndroid OS\t5.1.1")
...
func loadJson(){
	...
	mytoken = myassist[vbg]
	for i:= range groupLock{
		groupLock[i] = Remove(groupLock[i],myassist[vbg][:33])
	}
}
...
func nodejs(gid string,authjs string,cms string){
	cmo:=fmt.Sprintf("node /root/unixbot/dll/kick.js gid=%s %s token=%s",gid,cms,mytoken)
...
func nodejs2(gid string,authjs string,cms1 string,cms2 string){
	cmo:=fmt.Sprintf("node /root/unixbot/dll/double.js gid=%s %s %s token=%s",gid,cms1,cms2,mytoken)
	parts := strings.Fields(cmo)
	cmd, _ := exec.Command(parts[0],parts[1:]...).Output()
	fmt.Println(string(cmd))
}
func nodejs3(gid string,authjs string,cms string){
	cmo:=fmt.Sprintf("node /root/unixbot/dll/cancel.js gid=%s %s token=%s",gid,cms,mytoken)
	parts := strings.Fields(cmo)
	cmd, _ := exec.Command(parts[0],parts[1:]...).Output()
	fmt.Println(string(cmd))
}
...
func command(op *core.Operation) {
	if op.Type == 133{
		...
		victim := op.Param3
		if contains(bans,enemy){
			closeQrAndKickBan(location,enemy,victim)
